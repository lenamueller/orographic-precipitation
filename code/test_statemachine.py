...
from split_events import find_special_events
...
def test_standard_case():
    ...
    expected = [[1,2,3]]
    assert expected == find_special_events(test_list, trigger_level=3)
...
def test_wrong_type():
    ...
    with pytest.raises(TypeError):
        find_special_events(test_list, trigger_level=3)
def test_trigger_1():
    test_list = [0,1,0]
    with pytest.raises(ValueError):
        find_special_events(test_list, trigger_level=1)
    
def test_trigger_2():
    test_list = [0,0,1,0,0]
    expected = [[1]]
    assert expected == find_special_events(test_list, trigger_level=2)